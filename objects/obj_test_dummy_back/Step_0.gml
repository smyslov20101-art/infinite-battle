/// Step Event - obj_test_dummy_back
...
if (!variable_instance_exists(id, "damage")) damage = 1;
...
if (!variable_instance_exists(id, "enemy_type")) enemy_type = "test_dummy_back";
...
if (hp <= 0) {
    ...
    LOG("🔄 Задний манекен возрожден! HP: " + string(hp));
    exit;
}
...
if (move_speed_slow_timer > 0) {
    ...
    if (move_speed_slow_timer <= 0) {
        ...
        LOG_CAT("❄️ Замедление движения заднего манекена прошло, скорость восстановлена", "combat");
        move_speed_slow_timer = 0;
    }
}
...
if (attack_speed_slow_timer > 0) {
    ...
    if (attack_speed_slow_timer <= 0) {
        ...
        LOG_CAT("🐢 Замедление атаки заднего манекена прошло, скорость восстановлена", "combat");
        attack_speed_slow_timer = 0;
    }
}
...
if (bleed_stacks > 0) {
    ...
    if (bleed_timer <= 0) {
        ...
        LOG("🩸 ЗАДНИЙ МАНЕКЕН: Кровотечение -" + string(bleed_total_damage) + 
        ...
        if (hp <= 0) {
            ...
            attack_speed_slow_timer = 0;
            move_speed_slow_timer = 0;
            LOG("🔄 Задний манекен возрожден от кровотечения! HP: " + string(hp));
        } else {
            bleed_timer = 1.0;
        }
    }
...
if (burn_stacks > 0) {
    ...
    if (burn_timer <= 0) {
        ...
        if (burn_total_damage > 0) {
            ...
            LOG("🔥 ЗАДНИЙ МАНЕКЕН: Горение -" + string(burn_total_damage) + 
        ...
        if (hp <= 0) {
            ...
            attack_speed_slow_timer = 0;
            move_speed_slow_timer = 0;
            LOG("🔄 Задний манекен возрожден от горения! HP: " + string(hp));
        } else {
            burn_timer = 2.0;
        }
    }
...
if (ice_timer > 0) {
    ...
    if (ice_damage_timer <= 0) {
        if (ice_damage_per_tick > 0) {
            ...
            LOG_CAT("🧊 ЗАДНИЙ МАНЕКЕН: Лед -" + string(ice_damage_per_tick) + 
    ...
    if (hp <= 0) {
        ...
        attack_speed_slow_timer = 0;
        move_speed_slow_timer = 0;
        LOG("🔄 Задний манекен возрожден от льда! HP: " + string(hp));
    }
}
    ...
    if (big_bleed_timer <= 0) {
        ...
        if (big_bleed_damage_timer <= 0) {
            if (big_bleed_damage > 0) {
                hp -= big_bleed_damage;
                LOG_CAT("🩸💥 БОЛЬШОЕ КРОВОТЕЧЕНИЕ ЗАДНЕГО МАНЕКЕНА: -" + string(big_bleed_damage) + 
                ...
                if (hp <= 0) {
                    ...
                    attack_speed_slow_timer = 0;
                    move_speed_slow_timer = 0;
                    LOG("🔄 Задний манекен возрожден от большого кровотечения! HP: " + string(hp));
                }
            }
...
if (attack_timer <= 0) {
    ...
    if (instance_exists(closest_hero)) {
        ...
        LOG("==========================================");
        LOG("🗡️ ЗАДНИЙ МАНЕКЕН АТАКУЕТ ГЕРОЯ!");
        ...
        LOG("⚠️ Задний манекен: нет героев для атаки!");
    }
...
if (debug_effect_timer <= 0) {
    if (bleed_stacks > 0) {
        LOG("📊 [ЗАДНИЙ МАНЕКЕН] Кровотечение: " + string(bleed_stacks) + 
                          " стеков, урон/сек: " + string(bleed_stacks * bleed_damage_per_stack));
    }
    if (burn_stacks > 0) {
        LOG("📊 [ЗАДНИЙ МАНЕКЕН] Горение: " + string(burn_stacks) + 
                          " стеков, урон/2 сек: " + string(burn_stacks * burn_damage_per_stack));
    }
    if (ice_timer > 0) {
        LOG("📊 [ЗАДНИЙ МАНЕКЕН] Лед: " + string(ice_damage_per_tick) + " урона/сек");
    }
